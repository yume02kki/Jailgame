...
public class MapMaker
{
    ...
    private void doorLink(Node self, Node target, Directions direction)
    {
        ...
        IntVector2 mirrorPos = new IntVector2(
            Misc.wrapAround(normalPos.X, target.room.getPlayareaWidth()),
            Misc.wrapAround(normalPos.Y, target.room.getPlayareaHeight())
        );

        Directions mirror = TransformDirection.mirrorDirection[direction];
        if (direction == Directions.RIGHT || direction == Directions.DOWN)
        {
            IntVector2 tempPos = new IntVector2(normalPos);
            normalPos = mirrorPos;
            mirrorPos = tempPos;
        }

        Renders rendersOpen = new Renders("openAutoDoors", Icons.get("doorOpen"));
        self.room.setEntity(new Entity($"door_{Misc.enumToString(direction)}", normalPos,
            [new Open("doorOpen1"), rendersOpen], [Tags.Doorway]));

        target.room.setEntity(new Entity($"door_{Misc.enumToString(mirror)}", mirrorPos,
            [new Open("doorOpen2"), rendersOpen], [Tags.Doorway]));
    }

    public Entity getAutoGeneratedDoor(Room room, Directions? direction = null)
    {
        var doors = room.getEntityList().FindAll(entity => entity.tags.Contains(Tags.Doorway)).Select(entity => entity).ToList();
