...
{
    public static Dictionary<string, Action<List<Entity>?>> commander { get; } = new Dictionary<string, Action<List<Entity>?>>();
    private static Player player => GameCreator.Instance.player;

    static CommandManager()
    {
        void add(dynamic command, Action<List<Entity>> action)
        {
            commander.Add(command.ToString(), action);
        }

        add(Commands.open, operands => execute(typeof(Open), operands));
        add(Commands.use, operands => execute(typeof(Used), operands.LastOrDefault(), operands.FirstOrDefault()));
        add(Commands.examine, operands => execute(typeof(Examine), operands));
        ...
        add(Commands.load, _ => GameCreator.Instance.resetInstance(GameSaver.load<GameCreator>()!));
    ...
    private static void execute(Type type, params List<Entity> operands)
    {
        if (operands.Count == 0) return;
        if (operands.Count == 1)
        {
            operands.First().components.execute(type);
        }
        else
        {
            operands.First().components.execute(type, operands.Last());
        }
    }
    ...
    {
        var result = autoComplete(str, out var filter, lists);
    ...
    private static List<Entity> getOperands(string[] names)
    {
        ...
        foreach (var name in names)
        {
            ...
            Entity? roomOperand = player.currentNode.room.getEntity(nameFound);
