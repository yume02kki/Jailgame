...
public class GameInit
{
    ...
    private void doorLink(Room thisRoom, Room otherRoom, Direction direction)
    {
        ...
        thisRoom.setEntity(new Door($"door_{direction}", normalX, normalY, new Open()));
        otherRoom.setEntity(new Door($"door_{mirror}", mirrorX, mirrorY, new Open()));
    }

    
    public Door getAutoGeneratedDoor(Room room,Direction? direction=null)
    {
        List<Door> doors = room.getEntityList().FindAll(ent => ent is Door).Select(ent => (Door)ent).ToList();
